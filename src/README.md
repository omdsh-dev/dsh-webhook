...
- `src/store.ts`: durable-store domain facade; `src/store/` contains record types, schema codecs/migrations, and merge policy;
- `src/adapter.ts`: receipt-first Automation submission and durable Run-event reconciliation;
